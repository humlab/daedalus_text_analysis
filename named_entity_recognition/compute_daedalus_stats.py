...
def get_stored_classified_locations(filename):
    ...
    '''
    df = FileUtility.read_excel(filename, 'Sheet1')
...
def compute_words_per_year(source_folder):
    ...
    '''
    filenames = FileUtility.get_filenames(source_folder)
...
def get_word_count_per_decade(filename):
    df = FileUtility.read_excel(filename, 'Sheet1') #.reset_index()
...
if do_compute_word_stat:
    ...
    FileUtility.save_excel((df_word_stat, 'Sheet1'), word_count_filename)
else:
    df_word_stat = FileUtility.read_excel(word_count_filename, 'Sheet1').set_index('year')
...
FileUtility.save_excel((df_stat, 'Sheet1'), stat_filename)
