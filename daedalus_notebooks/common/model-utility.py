...
class ModelUtility:
    ...
    def get_excel_model(data_folder, basename):
        if len(ModelUtility.get_excel_models(data_folder).get(basename, {}).keys()) == 0:
            print('Loading {}, please wait...'.format(basename.upper()))
            ...
            print('Exporting CSV for {}, please wait...'.format(basename.upper()))
            ModelUtility.export_excel_to_csv(data_folder, basename, excel_model)
            print('Model {} loaded...'.format(basename.upper()))
    ...
    def get_corpus_documents(data_folder, basename):
        ...
        corpus_documents = pd.read_csv(os.path.join(data_folder, basename, 'corpus_documents.csv'), sep='\t', header=0)
